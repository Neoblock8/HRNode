...
public class Program {
  ...
  /**
   * @param transferAddress the address send WYZ to.
   * @param value the WYZ value transferred in the internal transaction
   */
  ...
  public void suicide(DataWord obtainerAddress) {
    ...
    if (FastByteComparisons.compareTo(owner, 0, 20, obtainer, 0, 20) == 0) {
      ...
      if (VMConfig.allowWvmTransferTrc10()) {
      ...
      try {
        ...
        if (VMConfig.allowWvmConstantinople()) {
          throw new TransferException(
              "transfer all token or transfer all wyz failed in suicide: %s", e.getMessage());
        }
  ...
  private void suicideFreezeBalanceAndVote(byte[] owner, byte[] obtainer, Repository repository) {
    ...
    if (FastByteComparisons.compareTo(obtainer, 0, 20,
      ...
      repository
              .addTotalNetWeight(-unfreezeBalance / Parameter.ChainConstant.WYZ_PRECISION);
    } else {
  ...
  private void createContractImpl(DataWord value, byte[] programCode, byte[] newAddress,
    ...
    if (VMConfig.allowWvmConstantinople()) {
      ...
      if (!contractAlreadyExists) {
        ...
        if (isCreate2) {
          builder.setWyzHash(ByteString.copyFrom(rootTransactionId));
        }
    ...
    if (byTestingSuite()) {
      ...
      if (!isTokenTransfer) {
        try {
          ...
          if (VMConfig.allowWvmConstantinople()) {
            refundEnergy(msg.getEnergy().longValue(), REFUND_ENERGY_FROM_MESSAGE_CALL);
            throw new TransferException("transfer wyz failed: %s", e.getMessage());
          }
          throw new BytecodeExecutionException(VALIDATE_FOR_SMART_CONTRACT_FAILURE, e.getMessage());
        }
        deposit.addBalance(senderAddress, -endowment);
  ...
  public void checkCPUTimeLimit(String opName) {
    ...
    if (vmNowInUs > getVmShouldEndInUs()) {
      logger.info(
          ...
          getVmShouldEndInUs(), vmNowInUs, gewvmStartInUs());
  ...
  public long gewvmStartInUs() {
    return this.invoke.gewvmStartInUs();
  }
  ...
  private void createAccountIfNotExist(Repository deposit, byte[] contextAddress) {
    if (VMConfig.allowWvmSolidity059()) {
      //after solidity059 proposal , allow contract transfer trc10 or WYZ to non-exist address(would create one)
