...
public class BlockMsgHandler implements HRNetworkMsgHandler {
  ...
  public void processMessage(PeerConnection peer, HRNetworkMessage msg) throws P2pException {
    ...
    if (peer.getSyncBlockRequested().containsKey(blockId)) {
      ...
      long interval = blockId.getNum() - HRNetworkNetDelegate.getHeadBlockId().getNum();
  ...
  private void processBlock(PeerConnection peer, BlockCapsule block) throws P2pException {
    ...
    if (!HRNetworkNetDelegate.containBlock(block.getParentBlockId())) {
      logger.warn("Get unlink block {} from {}, head is {}.", blockId.getString(),
          peer.getInetAddress(), HRNetworkNetDelegate.getHeadBlockId().getString());
    ...
    if (fastForward) {
      if (block.getNum() < HRNetworkNetDelegate.getHeadBlockId().getNum()) {
        logger.warn("Receive a low block {}, head {}",
            blockId.getString(), HRNetworkNetDelegate.getHeadBlockId().getString());
        return;
      }
      if (HRNetworkNetDelegate.validBlock(block)) {
        advService.fastForward(new BlockMessage(block));
        HRNetworkNetDelegate.trustNode(peer);
      }
    }

    HRNetworkNetDelegate.processBlock(block, false);
    witnessProductBlockService.validWitnessProductTwoBlock(block);
    HRNetworkNetDelegate.getActivePeer().forEach(p -> {
