...
@Slf4j(topic = "net")
@Component
public class ChainInventoryMsgHandler implements HRNetworkMsgHandler {
  ...
  public void processMessage(PeerConnection peer, HRNetworkMessage msg) throws P2pException {
    ...
    if (blockIdWeGet.size() == 1 && HRNetworkNetDelegate.containBlock(blockIdWeGet.peek())) {
    ...
    synchronized (HRNetworkNetDelegate.getBlockLock()) {
      while (!peer.getSyncBlockToFetch().isEmpty() && HRNetworkNetDelegate
  ...
  private void check(PeerConnection peer, ChainInventoryMessage msg) throws P2pException {
    ...
    if (HRNetworkNetDelegate.getHeadBlockId().getNum() > 0) {
      long maxRemainTime =
          ChainConstant.CLOCK_MAX_DELAY + System.currentTimeMillis() - HRNetworkNetDelegate
              .getBlockTime(HRNetworkNetDelegate.getSolidBlockId());
      long maxFutureNum =
          maxRemainTime / BLOCK_PRODUCED_INTERVAL + HRNetworkNetDelegate.getSolidBlockId().getNum();
