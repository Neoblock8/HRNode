...
public class FetchInvDataMsgHandler implements HRNetworkMsgHandler {
  ...
  public void processMessage(PeerConnection peer, HRNetworkMessage msg) throws P2pException {
    ...
    for (Sha256Hash hash : fetchInvDataMsg.getHashList()) {
      ...
      if (message == null) {
        try {
          message = HRNetworkNetDelegate.getData(hash, type);
        } catch (Exception e) {
  ...
  private void sendPbftCommitMessage(PeerConnection peer, BlockCapsule blockCapsule) {
    try {
      if (!HRNetworkNetDelegate.allowPBFT() || peer.isSyncFinish()) {
        return;
      }
      long epoch = 0;
      PbftSignCapsule pbftSignCapsule = HRNetworkNetDelegate
      ...
      if (epochCache.getIfPresent(epoch) == null) {
        PbftSignCapsule srl = HRNetworkNetDelegate.getSRLPbftCommitData(epoch);
        if (srl != null) {
  ...
  private void check(PeerConnection peer, FetchInvDataMessage fetchInvDataMsg) throws P2pException {
    ...
    if (type == MessageTypes.WYZ) {
      for (Sha256Hash hash : fetchInvDataMsg.getHashList()) {
        if (peer.getAdvInvSpread().getIfPresent(new Item(hash, InventoryType.WYZ)) == null) {
          throw new P2pException(TypeEnum.BAD_MESSAGE, "not spread inv: {}" + hash);
        }
      }
      int fetchCount = peer.getNodeStatistics().messageStatistics.tronInWyzFetchInvDataElement
          .getCount(10);
      int maxCount = advService.getWyzCount().getCount(60);
