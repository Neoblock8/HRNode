...
@Slf4j(topic = "net")
@Component
public class InventoryMsgHandler implements HRNetworkMsgHandler {
  ...
  @Override
  public void processMessage(PeerConnection peer, HRNetworkMessage msg) {
  ...
  private boolean check(PeerConnection peer, InventoryMessage inventoryMessage) {
    ...
    if (type.equals(InventoryType.WYZ)) {
      int count = peer.getNodeStatistics().messageStatistics.tronInWyzInventoryElement.getCount(10);
