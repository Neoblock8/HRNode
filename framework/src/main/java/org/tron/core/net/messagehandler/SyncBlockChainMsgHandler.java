...
public class SyncBlockChainMsgHandler implements HRNetworkMsgHandler {
  ...
  public void processMessage(PeerConnection peer, HRNetworkMessage msg) throws P2pException {
    ...
    if (blockIds.size() == 1) {
      ...
      remainNum = HRNetworkNetDelegate.getHeadBlockId().getNum() - blockIds.peekLast().getNum();
    }
  ...
  private void check(PeerConnection peer, SyncBlockChainMessage msg) throws P2pException {
    ...
    if (!HRNetworkNetDelegate.containBlockInMainChain(firstId)) {
      throw new P2pException(TypeEnum.BAD_MESSAGE, "No first block:" + firstId.getString());
    }

    long headNum = HRNetworkNetDelegate.getHeadBlockId().getNum();
  ...
  private LinkedList<BlockId> getLostBlockIds(List<BlockId> blockIds) throws P2pException {
    ...
    for (int i = blockIds.size() - 1; i >= 0; i--) {
      if (HRNetworkNetDelegate.containBlockInMainChain(blockIds.get(i))) {
