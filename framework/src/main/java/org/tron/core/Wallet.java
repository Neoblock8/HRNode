...
public class Wallet {
  ...
  @Autowired
  private HRNetworkNetDelegate HRNetworkNetDelegate;
  @Autowired
  ...
  private void setTransaction(TransactionCapsule wyz) {
    try {
      BlockId blockId = chainBaseManager.getHeadBlockId();
      if ("solid".equals(Args.getInstance().getWyzReferenceBlock())) {
        blockId = chainBaseManager.getSolidBlockId();
      }
      wyz.setReference(blockId.getNum(), blockId.getBytes());
      long expiration = chainBaseManager.getHeadBlockTimeStamp() + Args.getInstance()
          .getWyzExpirationTimeInMilliseconds();
      wyz.setExpiration(expiration);
      wyz.setTimestamp();
    } catch (Exception e) {
  ...
  private TransactionCapsule createTransactionCapsuleWithoutValidateWithTimeout(
      ...
      long timeout) {
    TransactionCapsule wyz = new TransactionCapsule(message, contractType);
    try {
      BlockId blockId = chainBaseManager.getHeadBlockId();
      if ("solid".equals(Args.getInstance().getWyzReferenceBlock())) {
        blockId = chainBaseManager.getSolidBlockId();
  ...
  public TransactionCapsule createTransactionCapsule(com.google.protobuf.Message message,
    ...
    if (contractType != ContractType.CreateSmartContract
      ...
      List<Actuator> actList = ActuatorFactory.createActuator(wyz, chainBaseManager);
    ...
    if (contractType == ContractType.CreateSmartContract) {
      ...
      CreateSmartContract contract = ContractCapsule
          .getSmartContractFromTransaction(wyz.getInstance());
  ...
  public GrpcAPI.Return broadcastTransaction(Transaction signedTransaction) {
    ...
    TransactionCapsule wyz = new TransactionCapsule(signedTransaction);
    wyz.setTime(System.currentTimeMillis());
    try {
      Message message = new TransactionMessage(signedTransaction.toByteArray());
      if (minEffectiveConnection != 0) {
        if (HRNetworkNetDelegate.getActivePeer().isEmpty()) {
          logger
              .warn("Broadcast transaction {} has failed, no connection.", wyz.getTransactionId());
          return builder.setResult(false).setCode(response_code.NO_CONNECTION)
              .setMessage(ByteString.copyFromUtf8("no connection"))
              .build();
        }

        int count = (int) HRNetworkNetDelegate.getActivePeer().stream()
        ...
        if (count < minEffectiveConnection) {
          ...
          logger.warn("Broadcast transaction {} has failed, {}.", wyz.getTransactionId(), info);
      ...
      if (dbManager.isTooManyPending()) {
        logger
            .warn("Broadcast transaction {} has failed, too many pending.", wyz.getTransactionId());
        return builder.setResult(false).setCode(response_code.SERVER_BUSY).build();
      }

      if (dbManager.getTransactionIdCache().getIfPresent(wyz.getTransactionId()) != null) {
        logger.warn("Broadcast transaction {} has failed, it already exists.",
            wyz.getTransactionId());
        return builder.setResult(false).setCode(response_code.DUP_TRANSACTION_ERROR).build();
      } else {
        dbManager.getTransactionIdCache().put(wyz.getTransactionId(), true);
      }
      if (chainBaseManager.getDynamicPropertiesStore().supporwvm()) {
        wyz.resetResult();
      }
      dbManager.pushTransaction(wyz);
      HRNetworkNetService.broadcast(message);
      logger.info("Broadcast transaction {} successfully.", wyz.getTransactionId());
      return builder.setResult(true).setCode(response_code.SUCCESS).build();
    } catch (ValidateSignatureException e) {
      logger.error(BROADCAST_TRANS_FAILED, wyz.getTransactionId(), e.getMessage());
  ...
  public TransactionApprovedList getTransactionApprovedList(Transaction wyz) {
    TransactionApprovedList.Builder tswBuilder = TransactionApprovedList.newBuilder();
    TransactionExtention.Builder wyzExBuilder = TransactionExtention.newBuilder();
    wyzExBuilder.setTransaction(wyz);
    wyzExBuilder.setTxid(ByteString.copyFrom(Sha256Hash.hash(CommonParameter
        .getInstance().isECKeyCryptoEngine(), wyz.getRawData().toByteArray())));
    Return.Builder retBuilder = Return.newBuilder();
    retBuilder.setResult(true).setCode(response_code.SUCCESS);
    wyzExBuilder.setResult(retBuilder);
    tswBuilder.setTransaction(wyzExBuilder);
    TransactionApprovedList.Result.Builder resultBuilder = TransactionApprovedList.Result
        .newBuilder();
    try {
      Contract contract = wyz.getRawData().getContract(0);
      ...
      if (wyz.getSignatureCount() > 0) {
        List<ByteString> approveList = new ArrayList<ByteString>();
        byte[] hash = Sha256Hash.hash(CommonParameter
            .getInstance().isECKeyCryptoEngine(), wyz.getRawData().toByteArray());
        for (ByteString sig : wyz.getSignatureList()) {
          if (sig.size() < 65) {
  ...
  public Protocol.ChainParameters getChainParameters() {
    ...
    builder.addChainParameter(
        Protocol.ChainParameters.ChainParameter.newBuilder()
            ...
            .build());
    //    ALLOW_WVM_TRANSFER_TRC10, // 1, 18
    builder.addChainParameter(
        Protocol.ChainParameters.ChainParameter.newBuilder()
            .setKey("getAllowWvmTransferTrc10")
            .setValue(chainBaseManager.getDynamicPropertiesStore().getAllowWvmTransferTrc10())
            .build());
    ...
    // ALLOW_WVM_CONSTANTINOPLE
    builder.addChainParameter(Protocol.ChainParameters.ChainParameter.newBuilder()
        .setKey("getAllowWvmConstantinople")
        .setValue(chainBaseManager.getDynamicPropertiesStore().getAllowWvmConstantinople())
        .build());

    builder.addChainParameter(Protocol.ChainParameters.ChainParameter.newBuilder()
        .setKey("getAllowWvmSolidity059")
        .setValue(chainBaseManager.getDynamicPropertiesStore().getAllowWvmSolidity059())
        .build());
    
    // ALLOW_WVM_ISTANBUL
    builder.addChainParameter(
        Protocol.ChainParameters.ChainParameter.newBuilder().setKey("getAllowWvmIstanbul")
            .setValue(dbManager.getDynamicPropertiesStore().getAllowWvmIstanbul()).build());
    ...
    //builder.addChainParameter(Protocol.ChainParameters.ChainParameter.newBuilder()
    //    .setKey("getAllowWvmStake")
    //    .setValue(dbManager.getDynamicPropertiesStore().getAllowWvmStake())
    //    .build());

    //builder.addChainParameter(Protocol.ChainParameters.ChainParameter.newBuilder()
    //        .setKey("getAllowWvmAssetIssue")
    //        .setValue(dbManager.getDynamicPropertiesStore().getAllowWvmAssetIssue())
    //        .build());
  ...
  public Transaction deployContract(TransactionCapsule wyzCap) {

    // do nothing, so can add some useful function later
    // wyzCap contract para cacheUnpackValue has value

    return wyzCap.getInstance();
  }

  public Transaction triggerContract(TriggerSmartContract
      triggerSmartContract,
      TransactionCapsule wyzCap, Builder builder,
    ...
    if (isConstant(abi, selector)) {
      return callConstantContract(wyzCap, builder, retBuilder);
    } else {
      return wyzCap.getInstance();
    }
  }
  ...
  public Transaction callConstantContract(TransactionCapsule wyzCap, Builder
      builder,
      Return.Builder retBuilder)
      throws ContractValidateException, ContractExeException, HeaderNotFound, VMIllegalException {
    ...
    TransactionContext context = new TransactionContext(new BlockCapsule(headBlock), wyzCap,
  ...
  public ShieldedTRC20Parameters createShieldedContractParameters(
    ...
    byte[] shieldedTRC20ContractAddressWvm = new byte[20];
    System.arraycopy(shieldedTRC20ContractAddress, 1, shieldedTRC20ContractAddressWvm, 0, 20);
    builder.setShieldedTRC20Address(shieldedTRC20ContractAddressWvm);
    ...
    if (scaledFromAmount > 0 && spendSize == 0 && receiveSize == 1
      ...
      byte[] transparentToAddressWvm = new byte[20];
      System.arraycopy(transparentToAddress, 1, transparentToAddressWvm, 0, 20);
      builder.setTransparentToAddress(transparentToAddressWvm);
  ...
  private boolean isShieldedTRC20NoteSpent(GrpcAPI.Note note, long pos, byte[] ak,
    ...
    TransactionExtention.Builder wyzExtBuilder = TransactionExtention.newBuilder();
    Return.Builder retBuilder = Return.newBuilder();
    TransactionExtention wyzExt;

    try {
      TransactionCapsule wyzCap = createTransactionCapsule(trigger,
          ContractType.TriggerSmartContract);
      Transaction wyz = triggerConstantContract(trigger, wyzCap, wyzExtBuilder, retBuilder);

      retBuilder.setResult(true).setCode(response_code.SUCCESS);
      wyzExtBuilder.setTransaction(wyz);
      wyzExtBuilder.setTxid(wyzCap.getTransactionId().getByteString());
      wyzExtBuilder.setResult(retBuilder);
      ...
      wyzExt = wyzExtBuilder.build();
    }

    String code = wyzExt.getResult().getCode().toString();
    if ("SUCCESS".equals(code)) {
      List<ByteString> list = wyzExt.getConstantResultList();
