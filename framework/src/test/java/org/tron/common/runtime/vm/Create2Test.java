...
public class Create2Test extends VMTestBase {
  ...
  */
  ...
  public void testCreate2()
    ...
    manager.getDynamicPropertiesStore().saveAllowWvmTransferTrc10(1);
    manager.getDynamicPropertiesStore().saveAllowWvmConstantinople(1);
    manager.getDynamicPropertiesStore().saveAllowWvmIstanbul(0);
    ...
    // deploy contract
    Transaction wyz = WvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractName, address, abi, factoryCode, value, fee, consumeUserResourcePercent,
        null);
    byte[] factoryAddress = WalletUtil.generateContractAddress(wyz);
    runtime = WvmTestUtils.processTransactionAndReturnRuntime(wyz, rootDeposit, null);
...
contract A {
  ...
  public void create2AddressTest() throws ContractExeException, ReceiptCheckErrException,
    ...
    manager.getDynamicPropertiesStore().saveAllowWvmTransferTrc10(1);
    manager.getDynamicPropertiesStore().saveAllowWvmConstantinople(1);
    manager.getDynamicPropertiesStore().saveAllowWvmSolidity059(1);
    manager.getDynamicPropertiesStore().saveAllowWvmIstanbul(1);
    String contractName = "Factory_0";
    byte[] address = Hex.decode(OWNER_ADDRESS);
    String abi = "[]";
    ...
    // same input
    result = WvmTestUtils.triggerContractAndReturnWvmTestResult(Hex.decode(OWNER_ADDRESS),
              factoryAddress, Hex.decode(hexInput), 0, fee, manager, null);
    ...
    result = WvmTestUtils
        .triggerContractAndReturnWvmTestResult(Hex.decode(ownerAddress2),
            factoryAddress, Hex.decode(hexInput), 0, fee, manager, null);
