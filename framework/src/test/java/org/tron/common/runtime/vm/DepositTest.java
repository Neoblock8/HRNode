...
public class DepositTest {
  ...
  public void loopCallTest()
    ...
    Transaction aWyz = WvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractA, address, aABI, aCode, value, fee, consumeUserResourcePercent,
        null, engeryLiimt);
    Runtime runtime = WvmTestUtils
        .processTransactionAndReturnRuntime(aWyz, DepositImpl.createRoot(manager),
            null);
    Assert.assertNull(runtime.getRuntimeError());

    Transaction bWyz = WvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractB, address, bABI, bCode, value, fee, consumeUserResourcePercent,
        null, engeryLiimt);
    runtime = WvmTestUtils
        .processTransactionAndReturnRuntime(bWyz, DepositImpl.createRoot(manager), null);
    Assert.assertNull(runtime.getRuntimeError());

    byte[] aAddress = WalletUtil.generateContractAddress(aWyz);
    byte[] bAddress = WalletUtil.generateContractAddress(bWyz);
    ...
    byte[] triggerData = WvmTestUtils
        .parseAbi("callBcallARevert(address,uint256,uint256)", params1);
    WVMTestResult result = WvmTestUtils
        .triggerContractAndReturnWvmTestResult(Hex.decode(OWNER_ADDRESS),
            aAddress, triggerData, 0, fee, manager, null);
    Assert.assertNull(result.getRuntime().getRuntimeError());

    // check result
    // expected: n1 = 1; n2 = 0
    byte[] checkN1Data = WvmTestUtils.parseAbi("n1()", null);
    byte[] checkN2Data = WvmTestUtils.parseAbi("n2()", null);

    WVMTestResult checkN1 = WvmTestUtils
        .triggerContractAndReturnWvmTestResult(Hex.decode(OWNER_ADDRESS),
            aAddress, checkN1Data, 0, fee, manager, null);
    WVMTestResult checkN2 = WvmTestUtils
        .triggerContractAndReturnWvmTestResult(Hex.decode(OWNER_ADDRESS),
    ...
    triggerData = WvmTestUtils.parseAbi("callBcallA(address,uint256,uint256)", params2);
    result = WvmTestUtils
        .triggerContractAndReturnWvmTestResult(Hex.decode(OWNER_ADDRESS),
            aAddress, triggerData, 0, fee, manager, null);
    Assert.assertNull(result.getRuntime().getRuntimeError());
    checkN1 = WvmTestUtils
        .triggerContractAndReturnWvmTestResult(Hex.decode(OWNER_ADDRESS),
            aAddress, checkN1Data, 0, fee, manager, null);
    checkN2 = WvmTestUtils
        .triggerContractAndReturnWvmTestResult(Hex.decode(OWNER_ADDRESS),
            aAddress, checkN2Data, 0, fee, manager, null);
    System.out.println(Hex.toHexString(checkN1.getRuntime().getResult().getHReturn()));
    System.out.println(Hex.toHexString(checkN2.getRuntime().getResult().getHReturn()));
  ...
  public void loopCallTestOldVersion()
    ...
    Runtime runtime = WvmTestUtils.processTransactionAndReturnRuntime(aWyz, rootDeposit,
        null);
    Assert.assertNull(runtime.getRuntimeError());
    ...
    Transaction bWyz = WvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null);
    rootDeposit = DepositImpl.createRoot(manager);
    runtime = WvmTestUtils.processTransactionAndReturnRuntime(bWyz, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());
