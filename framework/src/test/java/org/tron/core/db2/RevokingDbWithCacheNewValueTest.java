...
public class RevokingDbWithCacheNewValueTest {
  ...
  public synchronized void testPop() throws RevokingStoreIllegalStateException {
    ...
    HRNetworkDatabase = new TestRevokingTronStore("testRevokingDBWithCacheNewValue-testPop");
  ...
  public synchronized void testMerge() {
    revokingDatabase = context.getBean(SnapshotManager.class);
    revokingDatabase.enable();
    HRNetworkDatabase = new TestRevokingTronStore("testRevokingDBWithCacheNewValue-testMerge");
    revokingDatabase.add(HRNetworkDatabase.getRevokingDB());
    ...
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      HRNetworkDatabase.put(testProtoCapsule.getData(), testProtoCapsule2);
      tmpSession.merge();
    }
    Assert.assertEquals(testProtoCapsule2, HRNetworkDatabase.get(testProtoCapsule.getData()));

    try (ISession tmpSession = revokingDatabase.buildSession()) {
      HRNetworkDatabase.delete(testProtoCapsule.getData());
      tmpSession.merge();
    }
    Assert.assertNull(HRNetworkDatabase.get(testProtoCapsule.getData()));
    dialog.reset();
  }
  ...
  public synchronized void testRevoke() {
    revokingDatabase = context.getBean(SnapshotManager.class);
    revokingDatabase.enable();
    HRNetworkDatabase = new TestRevokingTronStore("testRevokingDBWithCacheNewValue-testRevoke");
    revokingDatabase.add(HRNetworkDatabase.getRevokingDB());
    ...
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      HRNetworkDatabase.put(testProtoCapsule.getData(), new ProtoCapsuleTest("revoke22".getBytes()));
      tmpSession.merge();
    }

    try (ISession tmpSession = revokingDatabase.buildSession()) {
      HRNetworkDatabase.put(testProtoCapsule.getData(), new ProtoCapsuleTest("revoke222".getBytes()));
      tmpSession.merge();
    }
    ...
    logger.info(
        "**********testProtoCapsule:" + (HRNetworkDatabase.getUnchecked(testProtoCapsule.getData()))
            .toString());
    Assert.assertEquals(testProtoCapsule, HRNetworkDatabase.get(testProtoCapsule.getData()));
  }
  ...
  public synchronized void testGetlatestValues() {
    revokingDatabase = context.getBean(SnapshotManager.class);
    revokingDatabase.enable();
    HRNetworkDatabase = new TestRevokingTronStore("testSnapshotManager-testGetlatestValues");
    revokingDatabase.add(HRNetworkDatabase.getRevokingDB());
    while (revokingDatabase.size() != 0) {
      revokingDatabase.pop();
    }
    ...
    Set<ProtoCapsuleTest> result = HRNetworkDatabase.getRevokingDB().getlatestValues(5).stream()
  ...
  public synchronized void testGetValuesNext() {
    revokingDatabase = context.getBean(SnapshotManager.class);
    revokingDatabase.enable();
    HRNetworkDatabase = new TestRevokingTronStore("testSnapshotManager-testGetValuesNext");
    revokingDatabase.add(HRNetworkDatabase.getRevokingDB());
    while (revokingDatabase.size() != 0) {
      revokingDatabase.pop();
    }
    ...
    Set<ProtoCapsuleTest> result =
        HRNetworkDatabase.getRevokingDB().getValuesNext(
  ...
  public synchronized void testGetKeysNext() {
    revokingDatabase = context.getBean(SnapshotManager.class);
    revokingDatabase.enable();
    HRNetworkDatabase = new TestRevokingTronStore("testSnapshotManager-testGetKeysNext");
    revokingDatabase.add(HRNetworkDatabase.getRevokingDB());
    while (revokingDatabase.size() != 0) {
      revokingDatabase.pop();
    }
    ...
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      HRNetworkDatabase.put(pairPriceKey2, testProtoCapsule);
      tmpSession.commit();
    }
    testProtoCapsule = new ProtoCapsuleTest(("getKeysNext1").getBytes());
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      HRNetworkDatabase.put(pairPriceKey1, testProtoCapsule);
      tmpSession.commit();
    }

    testProtoCapsule = new ProtoCapsuleTest(("getKeysNext0").getBytes());
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      HRNetworkDatabase.put(pairPriceKey0, testProtoCapsule);
      tmpSession.commit();
    }
    testProtoCapsule = new ProtoCapsuleTest(("getKeysNext3").getBytes());
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      HRNetworkDatabase.put(pairPriceKey3, testProtoCapsule);
      tmpSession.commit();
    }

    List<byte[]> result = HRNetworkDatabase.getRevokingDB().getKeysNext(pairPriceKey0, 4);
  ...
  public synchronized void testGetKeysNextWithSameKey() {
    revokingDatabase = context.getBean(SnapshotManager.class);
    revokingDatabase.enable();
    HRNetworkDatabase = new TestRevokingTronStore("testSnapshotManager-testGetKeysNextWithSameKey");
    revokingDatabase.add(HRNetworkDatabase.getRevokingDB());
    while (revokingDatabase.size() != 0) {
      revokingDatabase.pop();
    }
    ...
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      HRNetworkDatabase.put(pairPriceKey2, testProtoCapsule2);
      tmpSession.commit();
    }
    Assert.assertArrayEquals(testProtoCapsule2.getData(),
        HRNetworkDatabase.get(pairPriceKey2).getData());

    ProtoCapsuleTest testProtoCapsule1 = new ProtoCapsuleTest(("getKeysNext1").getBytes());
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      HRNetworkDatabase.put(pairPriceKey1, testProtoCapsule1);
      tmpSession.commit();
    }
    ...
    Assert.assertArrayEquals(testProtoCapsule1.getData(),
        HRNetworkDatabase.get(pairPriceKey1).getData());
    Assert.assertArrayEquals(testProtoCapsule1.getData(),
        HRNetworkDatabase.get(pairPriceKey2).getData());

    ProtoCapsuleTest testProtoCapsule0 = new ProtoCapsuleTest(("getKeysNext0").getBytes());
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      HRNetworkDatabase.put(pairPriceKey0, testProtoCapsule0);
      tmpSession.commit();
    }

    ProtoCapsuleTest testProtoCapsule3 = new ProtoCapsuleTest(("getKeysNext3").getBytes());
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      HRNetworkDatabase.put(pairPriceKey3, testProtoCapsule3);
      tmpSession.commit();
    }

    List<byte[]> result = HRNetworkDatabase.getRevokingDB().getKeysNext(pairPriceKey0, 3);
  ...
  public static class TestRevokingTronStore extends HRNetworkStoreWithRevoking<ProtoCapsuleTest> {
