...
public class TransactionCapsuleTest {
  ...
  private static HRNetworkApplicationContext context;
  ...
  public void addSign() {
    ...
    Transaction.Builder wyzBuilder = Transaction.newBuilder();
    ...
    wyzBuilder.setRawData(rawBuilder);
    AccountStore accountStore = dbManager.getAccountStore();
    TransactionCapsule transactionCapsule = new TransactionCapsule(wyzBuilder.build());
    //Accout not exist
    try {
  ...
  public void validateSignature0() {
    ...
    //SignatureFormatException
    ByteString test = ByteString.copyFromUtf8("test");
    wyzBuilder.clearSignature().addSignature(test);
    try {
      TransactionCapsule.validateSignature(wyzBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
      Assert.assertFalse(true);
    } catch (PermissionException e) {
    ...
    new Random().nextBytes(rand);
    rand[64] = 8;  // v = 8 < 27 v += 35 > 35
    wyzBuilder.clearSignature().addSignature(ByteString.copyFrom(rand));
    try {
      TransactionCapsule.validateSignature(wyzBuilder.build(), hash,
          accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
      Assert.assertEquals(e.getMessage(), "Header byte out of range: 35");
    ...
    wyzBuilder.clearSignature().addAllSignature(sign21_11);
    try {
      TransactionCapsule.validateSignature(wyzBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
    ...
    wyzBuilder.clearSignature().addAllSignature(sign21_11_22_23);
    try {
      TransactionCapsule
          .validateSignature(wyzBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
      Assert.assertFalse(true);
    } catch (PermissionException e) {
    ...
    wyzBuilder.clearSignature().addAllSignature(sign21_22_21);
    try {
      TransactionCapsule
          .validateSignature(wyzBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    ...
    wyzBuilder.clearSignature().addAllSignature(sign21);
    try {
    ...
    wyzBuilder.clearSignature().addAllSignature(sign21_22);
    try {
      boolean result = TransactionCapsule
          .validateSignature(wyzBuilder.build(), hash, accountStore);
    ...
    wyzBuilder.clearSignature().addAllSignature(sign21_22_23);
    try {
      boolean result = TransactionCapsule
          .validateSignature(wyzBuilder.build(), hash, accountStore);
      Assert.assertTrue(result);
  ...
  public void validateSignature1() {
    ...
    ArrayList<ByteString> sign = sign(prikeys, Sha256Hash.hash(rawBuilder.build().toByteArray()));
    wyzBuilder.addAllSignature(sign);
    transactionCapsule = new TransactionCapsule(wyzBuilder.build());

    // no contract
    prikeys.clear();
    prikeys.add(ByteArray.fromHexString(KEY_21));
    wyzBuilder = Transaction.newBuilder();
    ...
    wyzBuilder = transactionCapsule.getInstance().toBuilder();
    //SignatureFormatException
    ByteString test = ByteString.copyFromUtf8("test");
    wyzBuilder.clearSignature();
    wyzBuilder.addSignature(test);
    transactionCapsule = new TransactionCapsule(wyzBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    rand[64] = 8;  // v = 8 < 27 v += 35 > 35
    wyzBuilder.clearSignature();
    wyzBuilder.addSignature(ByteString.copyFrom(rand));
    transactionCapsule = new TransactionCapsule(wyzBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_11 = sign(prikeys, hash);
    wyzBuilder.clearSignature();
    wyzBuilder.addAllSignature(sign21_11);
    transactionCapsule = new TransactionCapsule(wyzBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_11_22_23 = sign(prikeys, hash);
    wyzBuilder.clearSignature();
    wyzBuilder.addAllSignature(sign21_11_22_23);
    transactionCapsule = new TransactionCapsule(wyzBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_22_21 = sign(prikeys, hash);
    wyzBuilder.clearSignature();
    wyzBuilder.addAllSignature(sign21_22_21);
    transactionCapsule = new TransactionCapsule(wyzBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21 = sign(prikeys, hash);
    wyzBuilder.clearSignature();
    wyzBuilder.addAllSignature(sign21);
    transactionCapsule = new TransactionCapsule(wyzBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_22 = sign(prikeys, hash);
    wyzBuilder.clearSignature();
    wyzBuilder.addAllSignature(sign21_22);
    transactionCapsule = new TransactionCapsule(wyzBuilder.build());
    try {
    ...
    ArrayList<ByteString> sign21_22_23 = sign(prikeys, hash);
    wyzBuilder.clearSignature();
    wyzBuilder.addAllSignature(sign21_22_23);
    transactionCapsule = new TransactionCapsule(wyzBuilder.build());
    try {
      boolean result = transactionCapsule.validateSignature(dbManager);
      Assert.assertTrue(result);
  ...
  @Test
  public void wyzCapsuleClearTest() {
    Transaction tx = Transaction.newBuilder()
        .addRet(Result.newBuilder().setContractRet(contractResult.OUT_OF_TIME).build()).build();
    TransactionCapsule wyzCap = new TransactionCapsule(tx);
    Result.contractResult contractResult = wyzCap.getContractResult();
    wyzCap.resetResult();
    Assert.assertEquals(wyzCap.getInstance().getRetCount(), 0);
    wyzCap.setResultCode(contractResult);
    Assert.assertEquals(wyzCap.getInstance()
