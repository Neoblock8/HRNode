...
public class AdvServiceTest {
  ...
  protected HRNetworkApplicationContext context;
  ...
  private void testAddInv() {
    boolean flag;
    Item itemWyz = new Item(Sha256Hash.ZERO_HASH, InventoryType.WYZ);
    flag = service.addInv(itemWyz);
  ...
  private void testWyzBroadcast() {
    Protocol.Transaction wyz = Protocol.Transaction.newBuilder().build();
    CommonParameter.getInstance().setValidContractProtoThreadNum(1);
    TransactionMessage msg = new TransactionMessage(wyz);
    service.broadcast(msg);
    Item item = new Item(msg.getMessageId(), InventoryType.WYZ);
    Assert.assertNotNull(service.getMessage(item));
  }
