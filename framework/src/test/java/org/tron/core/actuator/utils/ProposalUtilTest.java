...
public class ProposalUtilTest {
  ...
  public void validProposalTypeCheck() throws ContractValidateException {
    ...
    Assert.assertEquals(ProposalType.ALLOW_WVM_SOLIDITY_059, ProposalType.getEnumOrNull(32));
    ...
    code = 32;
    Assert.assertEquals(ProposalType.ALLOW_WVM_SOLIDITY_059, ProposalType.getEnum(code));
  ...
  public void validateCheck() {
    ...
    try {
      actuatorUtil.validator(dynamicPropertiesStore, forkUtils,
          ProposalType.ALLOW_WVM_TRANSFER_TRC10.getCode(), 2);
      Assert.assertTrue(false);
    } catch (ContractValidateException e) {
      Assert.assertEquals(
          "This value[ALLOW_WVM_TRANSFER_TRC10] is only allowed to be 1", e.getMessage());
    }

    dynamicPropertiesStore.saveAllowSameTokenName(0);
    try {
      actuatorUtil.validator(dynamicPropertiesStore, forkUtils,
          ProposalType.ALLOW_WVM_TRANSFER_TRC10.getCode(), 1);
      Assert.assertTrue(false);
    } catch (ContractValidateException e) {
      Assert.assertEquals("[ALLOW_SAME_TOKEN_NAME] proposal must be approved "
          + "before [ALLOW_WVM_TRANSFER_TRC10] can be proposed", e.getMessage());
    }
