...
public class NullifierStoreTest {
  ...
  private static final byte[] WYZ_TWO = randomBytes(32);
  private static final byte[] WYZ_TWO_NEW = randomBytes(32);
  ...
  public static void init() {
    ...
    nullifier2 = new BytesCapsule(WYZ_TWO);
    nullifier2New = new BytesCapsule(WYZ_TWO_NEW);
  ...
  public void putAndGet() {
    ...
    Assert.assertArrayEquals("putAndGet2", nullifier, WYZ_TWO);

    nullifierStore.put(NULLIFIER_TWO, nullifier2New);
    nullifier = nullifierStore.get(NULLIFIER_TWO).getData();
    Assert.assertArrayEquals("putAndGet2New", nullifier, WYZ_TWO_NEW);
  }
