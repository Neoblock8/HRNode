...
public class MarketPairPriceToOrderStoreWithCacheOldValueTest {
  ...
  public synchronized void testGetKeysNext() {
    ...
    TestRevokingTronStore HRNetworkDatabase = new TestRevokingTronStore(dbName, options,
        revokingDatabase);
    ...
    // put: 2 1 0 3
    Assert.assertFalse(HRNetworkDatabase.has(pairPriceKey2));
    HRNetworkDatabase.put(pairPriceKey2, capsule2);
    ...
    Assert.assertTrue(HRNetworkDatabase.has(pairPriceKey1));
    HRNetworkDatabase.put(pairPriceKey1, capsule1);
    Assert.assertEquals(1, HRNetworkDatabase.size());
    ...
    Assert.assertFalse(HRNetworkDatabase.has(pairPriceKey0));
    if (!HRNetworkDatabase.has(pairPriceKey0)) {
      HRNetworkDatabase.put(pairPriceKey0, capsule0);
    }

    Assert.assertEquals(2, HRNetworkDatabase.size());

    Assert.assertFalse(HRNetworkDatabase.has(pairPriceKey3));
    if (!HRNetworkDatabase.has(pairPriceKey3)) {
      HRNetworkDatabase.put(pairPriceKey3, capsule3);
    }

    Assert.assertEquals(3, HRNetworkDatabase.size());

    // get pairPriceKey1, will get pairPriceKey2's value capsule2
    try {
      Assert
          .assertArrayEquals(capsule0.getData(),
              HRNetworkDatabase.get(pairPriceKey0).getData());
      Assert
          .assertArrayEquals(capsule1.getData(),
              HRNetworkDatabase.get(pairPriceKey1).getData());
      Assert
      ...
      Assert
          .assertArrayEquals(capsule3.getData(),
              HRNetworkDatabase.get(pairPriceKey3).getData());
    } catch (ItemNotFoundException | BadItemException e) {
      Assert.fail();
    }

    List<byte[]> keyList = HRNetworkDatabase.getRevokingDB().getKeysNext(pairPriceKey0, 2 + 1);
  ...
  private static class TestRevokingTronStore extends
      HRNetworkStoreWithRevoking<MarketOrderIdListCapsule> {
